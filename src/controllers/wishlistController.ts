...
export async function getAddProductToWishlist(req: IRequestWithAuthenticatedUser, res: Response, next: NextFunction) {
    const productId = parseInt(req.params.productId);

    const user = await User.findByPk(req.user.id);
    
    let userWishlist = user?.wishlist;
    if (user === null || userWishlist?.includes(productId)) {
        res.redirect('back');
        return;
    }
    ...
    await User.update({ wishlist: userWishlist  }, {
        where: {
            id: req.user.id,
        },
    });
...
export async function getRemoveProductFromWishlist(req: IRequestWithAuthenticatedUser, res: Response, next: NextFunction) {
    const productId = parseInt(req.params.productId);
